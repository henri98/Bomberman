...
{
  for (uint8_t i = 0; i < ARRAY_SIZE(getOpponentBombPos()); i++)
  {
    if (getOpponentBombPos()[i].is_used == 0)
    {
      opponent->bomblist[i].location_x = getOpponentBombPos()[i].location_x;
      opponent->bomblist[i].location_y = getOpponentBombPos()[i].location_y;
